...
class AlarmDB(Base):
    ...
    alarm_type = Column(Integer, nullable=False)  # 0-安全规范（未戴安全帽/未穿反光衣） 2-区域入侵（人/车） 3-火警（火焰/烟雾）
    alarm_status = Column(Integer, default=0)  # 0-未处理 1-确认误报 2-处理中（已派单） 3-处理完成
    alarm_time = Column(DateTime, nullable=False) # 告警触发时间，非空
    snapshot_url = Column(String(255), nullable=False) # 报警截图URL，非空
    video_clip_url = Column(String(255), nullable=False) # 报警视频片段URL，非空
